    ...
    def paintEvent(self, event):
        ...
        if 0 <= self.tank_level < 0.26:
            ...
            if self.tank_level==0:
                painter.drawText(tank_x-20, tank_y + tank_height-322, f"Tank Level={int(self.tank_level * 100)}% : EMPTY TANK")
            else:
                painter.drawText(tank_x-20, tank_y + tank_height-322, f"Tank Level={int(self.tank_level * 100)}% : CRITICAL")
        elif 0.25 <= self.tank_level < 0.51:
            painter.setPen(QColor("#E4670B"))
            painter.setBrush(QColor("#E4670B"))
            painter.drawText(tank_x-20, tank_y + tank_height-322, f"Tank Level={int(self.tank_level * 100)}% : MODERATE")
        elif 0.5 <= self.tank_level < 0.76:
            painter.setPen(QColor("#EBA104"))
            painter.setBrush(QColor("#EBA104"))
            painter.drawText(tank_x-20, tank_y + tank_height-322, f"Tank Level={int(self.tank_level * 100)}% : GOOD")
        elif 0.75 <= self.tank_level <= 1.0:
            painter.setPen(QColor("#94C816"))
            painter.setBrush(QColor("#94C816"))
            if self.tank_level==1:
                painter.drawText(tank_x-20, tank_y + tank_height-322, f"Tank Level={int(self.tank_level * 100)}% : FULL TANK")
            else:
                painter.drawText(tank_x-20, tank_y + tank_height-322, f"Tank Level={int(self.tank_level * 100)}% : HIGH")
